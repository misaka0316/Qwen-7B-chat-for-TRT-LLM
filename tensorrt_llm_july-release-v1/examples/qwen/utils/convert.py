...
def split_and_save_weight(tp_rank, saved_dir, split_factor, key, vals,
    ...
    if "input_layernorm.weight" in key or "input_layernorm.bias" in key or \
    ...
    elif "attention.dense.weight" in key or "mlp.dense_4h_to_h.weight" in key or "mlp.dense_w2_to_4h.weight" in key:
        cat_dim = 0
    ...
    elif "mlp.dense_h_to_4h.weight" in key or "mlp.dense_h_to_4h.bias" in key or "mlp.dense_w1_to_4h.weight" in key :
        ...
        if multi_query_mode:
            ...
            vals = [val.reshape(hidden_dim, vals[0].shape[-1]) for val in vals]
            cat_dim = -1
            val = np.concatenate(vals, axis=cat_dim)
            split_vals = np.split(val, split_factor, axis=cat_dim)
        save_split(split_vals, saved_dir, key, tp_rank, split_factor)
        ...
        pass
    # elif "mlp.dense_w1_to_4h.weight" in key:
    #     print("key")
    # elif "mlp.dense_w1_to_4h.weight" in key:
    #     print("key")
