...
import sys
import nvtx
sys.path.append("/root/workspace/tensorrt_llm_july-release-v1/examples/qwen/qwen")
from qwen.qwen_generation_utils import (
    HistoryType,
    make_context,
    decode_tokens,
    get_stop_words_ids,
...
def generate(
    ...
    output_csv: str = None,
    output_npy: str = None,
    tokenizer_path: str = '/root/workspace/tensorrt_llm_july-release-v1/examples/qwen/qwen',
    vocab_file=None,
    num_beams: int = 1,
    ...
    # input_ids_ = torch.tensor([context_tokens])
    # np.save('input_ids.npy', input_ids.cpu())
    # input_lengths = torch.cuda.IntTensor([input_ids_.size(1)])

    input_tokens = []
    input_tokens.append(context_tokens)
    ...
    # with nvtx.annotate("trt-llm",color="red"):
